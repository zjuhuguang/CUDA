...
int main(int argc, char** argv) {
	...
	// for (int i = 0; i < P.height * P.width; ++i) printf("%f ", P.elements[i] - reference.elements[i]); printf("\n");
	printf("Test %s\n", (1 == res) ? "PASSED" : "FAILED");
...
{	
	...
	Matrix Nt = AllocateMatrix(N.width, N.height, 0);
	for (int i = 0; i < N.height; ++i)
		for (int j = 0;j < N.width; ++j)
			Nt.elements[j * Nt.width + i] = N.elements[i * N.width + j];
	CopyToDeviceMatrix(Md, M);
	Matrix Nd = AllocateDeviceMatrix(Nt);
	CopyToDeviceMatrix(Nd, Nt);
	...
	dim3 dimBlock(t_width, t_width);
	dim3 dimGrid((N.width - 1 + TILE_WIDTH) / TILE_WIDTH,(M.height - 1 + TILE_WIDTH) / TILE_WIDTH);
	
	MatrixMulKernel<<<dimGrid, dimBlock>>>(Md, Nd, Pd);



	//printf("%d %d %d\n", Md.height, Md.width, Nd.height);
